...
while (( "$#" )); do
    case "${1}" in
        ...
        -a|--action)
            ...
            ;;
	-json)
	    export tf_json=$(parameter_value --action "${2}")
	    shift 2
	    ;;
        --clone-launchpad)
            export caf_command="clone"
...
echo "DEBUG ===> caf_command: <${caf_command}>, tf_command: <${tf_command}>"
...
if [ "${caf_command}" != "walkthrough" ]; then
  ...
  echo "terraform plan json file      : '$(echo ${tf_json})'"
