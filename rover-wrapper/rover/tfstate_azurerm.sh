function initialize_state {
    ...
    echo "DEBUG tf_action: <${tf_action}>"
...
function plan {
    ...
    if [ "${tf_json}" != "" ]; then
         terraform show -json ${tf_plan_file} > ${tf_json} 
    fi
}
...
function show_json {
    echo "@calling show_json"

    echo "running terraform ${tf_action} ${tf_command} 2>$STDERR_FILE | tee ${tf_output_file}"
